...
class GitlabConnector:
    ...
    def file(self, path:str):
        ...
        resp = requests.get(f"{self.host}/api/v4/projects/{self.project}/respository/files/{quoted_path}/raw?ref={self.ref}")
    ...
    def ls(self):
        ...
        progress=None
        while True:
            resp = requests.get(f"{self.host}/api/v4/projects/{self.project}/repository/tree?ref={self.ref}&per_page={PER_PAGE}&page={page}&recursive=true")
            resp.raise_for_status()
            page = page + 1
            arr = resp.json()
            ...
            if tqdm_imported:
                if not progress:
                    progress = tqdm.tqdm(total=int(resp.headers.get("x-total-pages")),
                                        leave=True,
                                        unit="files")
                progress.update(1)
            if len(arr) == 0:
...
def traverse(obj: Union[Dict, List, str]) -> List[EbrainsIdValidationResult]:
    ...
    if isinstance(obj, dict):
        ...
        return [
            ...
            for err in traverse(value)
        ]
    if isinstance(obj, list):
        return [
            err 
            for item in obj
            for err in traverse(item)
        ]
...
def main():
    ...
    print(f"Scanned {len(all_results)}, Failed: {len(failed)}, Succeeded: {len(succeeded)}")
    if len(failed) > 0:
        raise Exception('\n'.join([f'filename={f.filename}, key={f.key}, value={f.value}' for f in failed]))
