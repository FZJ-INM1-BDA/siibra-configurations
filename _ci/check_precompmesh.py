...
def check_volume(arg):
    ...
    try:
        ...
        return (full_filname, vol_idx, volume, indices, ValidationResult.PASSED, None)
    except Exception as e:
        return (full_filname, vol_idx, volume, indices, ValidationResult.FAILED, str(e))
...
def main():
    ...
    print(
        f"PASSED: {len(passed)}, FAILED: {len(failed)}, SKIPPED: {len(skipped)}, TOTAL: {len(args)} {len(result)}"
    )
