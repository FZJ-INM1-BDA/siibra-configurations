...
def create_jba3_duplicate(
    ...
    if config.get('parcellation').get("@id") != "minds/core/parcellationatlas/v1.0.0/94c1125b-b87e-45e4-901c-00daee7f2579-290":
        return None
    else:
        config.get('parcellation')["@id"] = "minds/core/parcellationatlas/v1.0.0/94c1125b-b87e-45e4-901c-00daee7f2579-300"
    ...
    _ = config.get('ebrains').pop("openminds/DatasetVersion")  # replace this with dataset version uuid when it is released

    config.get("repository")["url"] = zip_url
    config['files'] = {sub: file.replace("294-Julich-Brain", str2replace) for sub, file in config.get('files').items()}
